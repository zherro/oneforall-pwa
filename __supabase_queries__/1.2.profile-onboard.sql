...
create policy "Users can insert their own profiles_onboard." on "public".profiles_onboard
  for insert with check ((select auth.uid()) = user_id);

create policy "Users can update own profiles_onboard." on "public".profiles_onboard
...
BEGIN
    ...
    WHERE profiles_onboard.user_id = NEW.id
    AND EXISTS (
        SELECT 1
        FROM jsonb_array_elements(steps) AS elem
        WHERE elem->>'id' = task_id AND (elem->>'completed')::boolean = false
    );
